...
FFI_SWIFT_DIR="${TARGET_DIR}/uniffi/swift"
...
# - `nostr_sdkFFI.h` must be moved in `target/uniffi/swift/include` dir
# - `nostr_sdkFFI.modulemap` must be renamed to `module.modulemap` and moved to `target/uniffi/swift/include` dir
# - `nostr_sdk.swift` must be renamed to NostrSDK.swift and moved to `target/uniffi/swift/Sources/NostrSDK` dir
#
...
mkdir -p "${FFI_HEADERS_DIR}"                                                               # Make `target/uniffi/swift/include` dir
mkdir -p "${FFI_SOURCES_DIR}/${PKG_NAME}"                                                   # Make `target/uniffi/swift/Sources/NostrSDK` dir
...
# Copy `target/uniffi/swift/Sources` dir to the Swift package
